          ...
          <div className="flex gap-2">
            {PHONES.map((p) => (
              <a
                ...
                className="bg-leaf-500 hover:bg-leaf-600 text-white text-sm font-medium px-3 py-1.5 rounded-full transition"
              >
                WhatsApp
              </a>
            ))}
      ...
      <section className="relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 py-12 md:py-20 flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1 text-center md:text-left">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-forest-800 leading-tight mb-4">
              Quality Seedlings.<br />
              <span className="text-leaf-500">Stronger Futures.</span>
            </h2>
            <p className="text-gray-600 text-lg mb-6 max-w-lg">
              Mukufuna mbande zabwino komanso zathanzi? Tili nazo pa mitengo yabwino. 
              Planting today, harvesting tomorrow.
            </p>
            <div className="flex flex-wrap gap-3 justify-center md:justify-start">
              <a
                href="#catalogue"
                className="bg-forest-600 hover:bg-forest-700 text-white font-semibold px-6 py-3 rounded-xl shadow-lg transition"
              >
                View Catalogue
              </a>
              <a
                href="#contact"
                className="border-2 border-forest-600 text-forest-700 hover:bg-forest-50 font-semibold px-6 py-3 rounded-xl transition"
              >
                Contact Us
              </a>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((p) => (
              <div
                key={p.id}
                className="bg-white rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden border border-forest-50"
              >
                <div className="relative h-48 bg-forest-50">
                  <Image
                    src={p.image}
                    alt={`${p.nameEn} seedlings`}
                    fill
                    className="object-cover"
                  />
                  {p.ready && (
                    <span className="absolute top-3 right-3 bg-leaf-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                      ✅ Ready
                    </span>
                  )}
                </div>
                <div className="p-5">
                  <h3 className="font-bold text-lg text-forest-800">
                    {p.nameCh} <span className="text-gray-500 font-normal text-base">({p.nameEn})</span>
                  </h3>
                  {p.variety && (
                    <p className="text-sm text-gray-500 mt-0.5">Variety: {p.variety}</p>
                  )}
                  <p className="text-2xl font-bold text-forest-600 mt-3">
                    K{p.price} <span className="text-sm font-normal text-gray-500">per seedling</span>
                  </p>
                  <button
                    onClick={() => {
                      setSelected(p.id);
                    ...
                    className="mt-4 w-full bg-forest-600 hover:bg-forest-700 text-white font-medium py-2.5 rounded-xl transition"
                  >
                    Order Now
                  </button>
                </div>
            ...
            {PHONES.map((p) => (
              <div key={p.number} className="bg-white rounded-2xl shadow-md p-6 flex-1 max-w-xs mx-auto">
                ...
                <div className="flex gap-2 justify-center">
                  <a
                    href={`tel:+265${p.number.slice(1)}`}
                    className="bg-forest-100 text-forest-700 hover:bg-forest-200 px-4 py-2 rounded-lg font-medium transition"
                  >
                    Call
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-leaf-500 hover:bg-leaf-600 text-white px-4 py-2 rounded-lg font-medium transition"
                  >
                    WhatsApp
                  </a>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-forest-100 text-forest-700 font-bold"
                >
                  −
                </button>
            ...
            <p className="text-lg font-semibold mb-4">
              Total: K{(product.price * qty).toLocaleString()}
            </p>
            ...
            <div className="flex flex-col gap-2">
              {PHONES.map((p) => (
                <button
                  key={p.number}
                  onClick={() => openWhatsApp(p.wa, `${product.nameCh} (${product.nameEn})`, qty)}
                  className="w-full bg-leaf-500 hover:bg-leaf-600 text-white font-medium py-3 rounded-xl transition"
                >
                  Order via {p.display}
                </button>
              ))}
